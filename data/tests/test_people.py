...
def temp_person():
    ret = ppl.create(TEMP_EMAIL, 'Joe Smith', 'Password123', 'NYU', [rls.TEST_CODE])
    yield ret
    ppl.delete(ret)
...
def test_delete():
    ppl.create(ANOTHER_EMAIL, "Jane Smith", 'Password123',"YOU", [rls.TEST_CODE])
    people = ppl.read()
    old_len = len(people)
...
def test_create():
    ...
    ppl.create(ADD_EMAIL, "John Smith", 'Password123',"WHO", [rls.TEST_CODE])
    people = ppl.read()
    assert ADD_EMAIL in people
    ppl.delete(ADD_EMAIL)
    ppl.create(TEST_EMAIL, 'David Bowie', 'Password123', 'Starman', [rls.TEST_CODE])
...
def test_create_duplicate():
    with pytest.raises(ValueError):
        ppl.create(ppl.TEST_EMAIL, "Name doesn't matter", 'Password123', "Affiliation doesn't matter", [rls.TEST_CODE])
...
def test_create_bad_email():
    with pytest.raises(ValueError):
        ppl.create("Bad email", "Name doesn't matter", 'Password123', "Affiliation doesn't matter", [rls.TEST_CODE])

def test_update_invalid_ppl():
    with pytest.raises(ValueError):
        ppl.update("Invalid email", "Unknown name", "Unknown affiliation", [rls.TEST_CODE])